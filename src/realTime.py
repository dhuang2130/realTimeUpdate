...
class App:
    ...
    def select_sales_file(self):
        self.sales_record_file_path = filedialog.askopenfilename(title="Select Sales Record File", filetypes=[("Excel files", "*.xlsx")])
        self.label1.config(text=f"Sales Record File: {self.sales_record_file_path}")

    def select_tracking_file(self):
        self.tracking_file_path = filedialog.askopenfilename(title="Select Tracking File", filetypes=[("Excel files", "*.xlsx")])
...
def update_tracking_file(sales_record_file_path, tracking_file_path):
    ...
    product_codes = tracking_df.iloc[:, 0].dropna().tolist()  

    # dictionary for product quantities
